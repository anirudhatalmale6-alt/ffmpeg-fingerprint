                                   ...
                                   uint8_t **out_buf, int *out_size)
{
    ...
    /* Render text into full-width bitmap (position baked in for IPTV/MAG compat) */
    int bmp_w, bmp_h;
    ...
    pes[p++] = 0x00; /* reserved */
    put_be16(pes + p, 0); p += 2; /* region_horizontal_address = 0 (full-width, position baked in bitmap) */
    ...
    put_be16(pes + p, bmp_w); p += 2; /* region_width = full display width */
